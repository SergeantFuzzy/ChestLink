...
public class InventoryMenu {
    ...
    public Inventory build(Player player, int page) {
        ...
        inv.setItem(49, nav(Material.OAK_SIGN, "&aYour linked chests", buildSummaryLore(player, chests)));
        return inv;
    }

    private ItemStack nav(Material mat, String name) {
        return nav(mat, name, null);
    }
    ...
    private ItemStack toIcon(BoundChest chest) {
        ...
        lore.add(messages.color("&7Usage: &f" + chest.getUsedSlots() + "/" + chest.getCapacity() + " Slots Used"));
    ...
    private List<String> buildSummaryLore(Player player, List<BoundChest> chests) {
        int total = chests.size();
        int inWorld = (int) chests.stream()
                .filter(b -> b.getLocation() != null && b.getLocation().getWorld() != null)
                .filter(b -> b.getLocation().getWorld().equals(player.getWorld()))
                .count();

        List<String> lore = new ArrayList<>();
        lore.add("&7Total linked chests: &f" + total);
        lore.add("&7This world: &f" + inWorld);
        if (total == 0) {
            lore.add("&8&m---------------------------&r");
            lore.add("&6How to create a linked chest");
            lore.add("&8&m---------------------------&r");
            lore.add("&7 1) &eHold a chest &7and run &6/cl bind");
            lore.add("&7 2) &ePlace &7the chest then &eleft-click &7it");
            lore.add("&7 3) &eOpen &7it with &6/cl open <id>");
            lore.add("&8&m---------------------------&r");
        }
        return lore;
    }
