...
public class MessageService {
    ...
    public void sendClickableLinked(Player player, BoundChest chest, List<String> hoverLore) {
        if (player == null || chest == null) {
            return;
        }
        TextComponent prefix = new TextComponent(color(getPrefix() + "&aLinked chest "));
        ...
        if (hoverLore != null && !hoverLore.isEmpty()) {
            ...
            name.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                    TextComponent.fromLegacyText(color("&eClick to open this linked chest"))));
        }

        TextComponent suffix = new TextComponent(color("&7 (#" + chest.getId() + ")."));
